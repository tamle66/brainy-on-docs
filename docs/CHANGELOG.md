...
- 2026-05-11 [Module 2] fix: UI Preview thiếu style toàn diện cho Markdown (Heading, Blockquote, Code...). Root cause: Tailwind CSS preflight xóa toàn bộ style mặc định của thẻ HTML, khiến `ReactMarkdown` render ra các thẻ `<h1-6>`, `<blockquote>`, `<code>` nhưng không có CSS đi kèm. Đã fix bằng cách bổ sung bộ CSS rule đầy đủ cho class `.markdown-content` tại `index.css`. Cập nhật thêm viền cho thẻ `table` và tinh chỉnh lại kích thước chuẩn cho các thẻ Heading.
- 2026-05-11 [Module 2] fix: Lỗi danh sách Menu Tab bị cuộn khuất khi nội dung bên trong quá dài. Root cause: Khung chứa ngoài cùng dùng `min-h-screen` khiến toàn bộ Add-on bị dài ra, dẫn đến scroll toàn trang. Khắc phục bằng cách khóa chiều cao `h-screen overflow-hidden` và chỉ cho phép thẻ `TabsContent` được phép cuộn (`overflow-y-auto min-h-0`).
- 2026-05-11 [Module 2] fix: Diện tích xem preview Markdown quá hẹp do nội dung bị ghim (pin) ở nửa trên màn hình. Root cause: Thẻ `SkillsActionTab` và `RewriteTab` bị gán `h-full` và bọc kết quả bên trong `<ScrollArea>`, dẫn đến chỉ khu vực Preview mới cuộn được. Đã tháo bỏ `<ScrollArea>` và `h-full`, trả quyền cuộn về cho thẻ `TabsContent` cha để toàn bộ nội dung tab cuộn nhịp nhàng.
